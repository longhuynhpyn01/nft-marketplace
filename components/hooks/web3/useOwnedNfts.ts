...
export const hookFactory: OwnedNftsHookFactory = ({ contract }) => () => {
    ...
    // Đưa NFT lên để sale trên marketplace
    // sử dụng useCallback để ngăn chặn việc tạo hàm lại nhiều lần
    const listNft = useCallback(async (tokenId: number, price: number) => {
        try {
            // tham số thứ 2 là newPrice nếu không truyền thì sẽ lấy giá niêm yết (listing price) như giá trị value trong option thứ 3
            // => gọi là giá cố định (fixed price): 0.025 ETH

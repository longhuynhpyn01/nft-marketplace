...
const NftCreate: NextPage = () => {
    ...
    const [nftURI, setNftURI] = useState(""); // check đã có nftURI sau khi upload metadata
    ...
    const getSignedData = async () => {
        // B1: Create messae to server
        const messageToSign = await axios.get("/api/verify");
        const accounts = await ethereum?.request({ method: "eth_requestAccounts" }) as string[];
        const account = accounts[0];

        // B2: Signature request
        const signedData = await ethereum?.request({
            method: "personal_sign",
            params: [JSON.stringify(messageToSign.data), account, messageToSign.data.id]
        }); // Trả về dạng hexa format
    ...
    const createNft = async () => {
        try {
            ...
            // transaction có thể mất một vài thời gian, đặc biệt khi build production
            await toast.promise(
                tx!.wait(),
                {
                    pending: "Uploading metadata",
